...
//! `SessionContext` (the name, not a specific struct), then anyone's
//! `AuthLayer` works with anyone's `CedarLayer`.
//!
//! # The convention
//!
//! | Name | Inserted by | Consumed by |
//! | --- | --- | --- |
//! | `SessionContext` | `AuthLayer` (or equivalent — anything that verifies a bearer token) | `CedarLayer`, `CedarInterceptor`, handler `require_session(ctx)?` |
...
//! SessionContext { ... }` in `example-multitenant-worker`,
//! `pub struct SessionContext { ... }` in `EdgeReplica`) and ALSO
//! exports the same name. That way, swapping the auth implementation
//! doesn't break Cedar.
//!
//! # Pattern: "soft middleware + handler backstop"
//!
//! See [`../../../MIDDLEWARES.md`](../../../MIDDLEWARES.md) §6 pattern
//! 3. Layers insert into extensions on success, do **nothing** on
...
//! than "everything 401".
//!
//! `CedarLayer` follows the same rule: missing `SessionContext` →
//! pass through (anonymous request, handler decides). Present
//! `SessionContext` + `Decision::Deny` in `Mode::Enforce` → short-circuit.
//!
